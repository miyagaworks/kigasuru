...
import { addShot, getSetting, saveSetting, getAllShots, clearAllData } from '../db';
import { setFlatThreshold } from '../sensors/gyro';
...
export const SettingsPage = () => {
  ...
  const loadThreshold = async () => {
    const saved = await getSetting('flatThreshold', 2);
    setThreshold(saved);
    setFlatThreshold(saved);
  };

  const loadClubs = async () => {
    const saved = await getSetting('customClubs', DEFAULT_CLUBS);
    setClubs(saved);
  };

  const loadInputSettings = async () => {
    const savedLevel = await getSetting('inputLevel', 'advanced');
    const savedFields = await getSetting('customInputFields', INPUT_LEVEL_PRESETS.advanced.fields);
  ...
  const handleInputLevelChange = async (level) => {
    setInputLevel(level);
    await saveSetting('inputLevel', level);
    if (level !== 'custom') {
      const fields = INPUT_LEVEL_PRESETS[level].fields;
  ...
  const handleCustomFieldToggle = async (field) => {
    const newFields = {
      ...customInputFields,
      [field]: !customInputFields[field],
    };
    setCustomInputFields(newFields);
    await saveSetting('customInputFields', newFields);
    await saveSetting('enabledInputFields', newFields);
  };
  ...
  return (
    <Layout>
      <div className="p-6">
        ...
        <div id="input-settings" className="bg-[var(--color-card-bg)] rounded-lg shadow-md p-6 mb-6">
